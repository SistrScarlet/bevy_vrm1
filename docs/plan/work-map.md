...
- `feat/absorb-ash-xr-vr-ik` — merge `3b6b6f1` (2026-07-12、8 commits)。batch-2: ash_xr `ik/` の VR IK VRM 知識層を bevy_vrm1 に移管 (solver / calibration / ECS systems)。公開 API: `VrIk` / `VrIkTargets` (外部ポーズ入力の切断面) / `VrIkChainCache` / `VrIkSystems` + solver 純粋関数 + `bone_names` 定数 (humanoid_bone)。`/code-review` finding 5 件解消 (`26b3727`、VR IK correctness + efficiency + simplification)。テスト 108 件 GREEN + clippy 0。ash_xr main (旧コード) の新 vrm1 main でのビルド確認済み。ash_xr 側の対応 branch `adopt-vrm1-vr-ik` も ash_xr main へマージ済み (`c69c09d5`、2026-07-13、4 commits)。`ik/` の重複実装 (解析 IK ソルバー・キャリブレーション・骨直接書込み) を除去し `VrIkTargets` へ毎フレーム転写する薄い adapter に一本化 (-1138/+95 行)。実行順前倒し (`GazeControl` → chain 先頭) の HMD smoke 実施済み (腕追従が自然化、悪影響なし)。引き継ぎ残: VR IK 診断ロギング (init 成功 info! + PERF_SNIFF) は vrm1 側に未復元 → ash_xr 側 roadmap 送り (vrm1 側での対応要否は未検討) → **batch-2 は両リポジトリで完了**
